;(function($){//uehtml.com && devolepMode
	var config = {};
	...
	function reset(){
		...
		/*
		config.inner = $('.designer_more').width();
		$('.designer_more a').width((config.inner-10)/3).height(((config.inner-10)/3)/280*180);
		*/
	...
	$('#content').bind('touchstart touchmove touchend',function(e){
		if($('#container').hasClass('push')==false && $('#container').hasClass('pull')==false){
			...
			if(e.type=='touchstart'){
				...
				/* iphone
				if($('#container').scrollTop()<10&&navigator.userAgent.match(/(iPhone|iPod|iPad)/i)){
					if(config.touchEY>50){
						//config.touchEY
						document.getElementById('header').style.webkitTransform='perspective(500px) rotateX('+config.touchEY/1.5+'deg)';
						if(config.touchEY/1.5<90){
							document.getElementById('header').style.webkitTransform='perspective(500px) rotateX('+config.touchEY/1.5+'deg)';
						}else{
							document.getElementById('header').style.webkitTransform='perspective(500px) rotateX(180deg)';
							$('.logo_msk').show();
						}
					}
				}
				*/
				...
				//iphone
				//document.getElementById('header').style.webkitTransform='';
				//$('.logo_msk').hide();
			}
		}
	});
	...
	$('.push_msk').bind('touchstart',function(e){
	...
	$('#add_f').click(function(){
		...
		if($(this).hasClass('liked')){
			var _this = $(this);
			$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,postid:commentId2},function(){
		...
		$('.relationship a').bind('click',function(){
			...
			if(_this.hasClass('need_login')){
				...
				if($(this).hasClass('add_friend')&&$(this).hasClass('need_login')==false){
					$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,uid:$(this).attr('data-uid')},function(){
	...
	/*方案*/
	var ff = ['','','',''],scLock;
	$('.ctf li').click(function(){				   
		ff[0] = $(this).attr('t_data').substring($(this).attr('t_data').indexOf('=')+1,$(this).attr('t_data').length);	
		$('.list2').html('<i></i><span>努力加载中...</span>');
		$.ajax({type:'get',url:'/html5/index.php?c=article&a=type&tid='+ff[0],success:function(data){
				var v = $(data).find('.listbody').html();
				$('#fuwu').addClass('hide');
				$('.swipe').addClass('hide');
				$('#list2').addClass('listbody');
				$('#list2').html(v);	
				is_lock=true;
				curcatid=ff[0];
		}})
	...
	/*服务*/
	var ff = ['','','',''],scLock;
	$('.ctfff li').click(function(){				   
		ff[0] = $(this).attr('t_data').substring($(this).attr('t_data').indexOf('=')+1,$(this).attr('t_data').length);		
		$('.list2').html('<i></i><span>努力加载中...</span>');
		$.ajax({type:'get',url:'/html5/index.php?c=article&a=type&tid='+ff[0],success:function(data){
				if(ff[0]==36){
					var v = $(data).find('.bangmang_list').html();
					$('#fuwu').addClass('hide');
					$('.swipe').addClass('hide');
					$('#list2').addClass('bangmang_list');
				}else{
					var v = $(data).find('.listbody').html();
					$('#fuwu').addClass('hide');
					$('.swipe').addClass('hide');
	...
	$('.ct li').click(function(){
		...
		if($(this).hasClass('a_selected')&&$(this).find('.s').attr('class')==undefined){
			var _this = $(this);
			dt[1] = '';
			$.get('/',{cat:dt[0],soindustrys:[dt[1]],sort:dt[2],search:search_value},function(data){
		...
		scLock = true,now_page = 1;//2014-1-16
	...
	$('#post_comment_btn').click(function(){
		if($('.post_comment_content').hasClass('show')){
			if(reply_lock==false){
				...
				if($('#post_comment_content').val()!=''){
					$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,postid:commentId2,content:$('#post_comment_content').val()},function(data){
		$('#comment ul').prepend('<li postdata="'+data.id+'" usdata="'+comment_id+'"><div class="pd5"><a class="avt fl" target="_blank" href="'+comment_at_url+'"><img src="'+comment_avatar+'"></a><div class="comment_content"><h5><div class="fl"><a class="comment_name" href="'+comment_at_url+'">'+comment_author+'</a><span>'+data.postdate+'</span></div><div class="fr reply_this">[回复]</div><div class="clear"></div></h5><div class="comment_p">'+data.content+'</div></div><div class="clear"></div><div class="comment_reply"></div></div></li>');
				...
				if($('#post_comment_content').val()!=''){
				//发送回复
				$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,postid:commentId2,postcid:_cid,postuid:comment_id,content:$('#post_comment_content').val()},function(data){
...
$('#comment ul').prepend('<li postdata="'+data.id+'" usdata="'+comment_id+'"><div class="pd5"><a class="avt fl" target="_blank" href="'+comment_at_url+'"><img src="'+comment_avatar+'"></a><div class="comment_content"><h5><div class="fl"><a class="comment_name" href="'+comment_at_url+'">'+comment_author+'</a><span>'+data.postdate+'</span></div><div class="fr reply_this">[回复]</div><div class="clear"></div></h5><div class="comment_p">'+data.content+ddd+'</div></div><div class="clear"></div><div class="comment_reply"></div></div></li>');
	...
	/*
	$('.post_comment_content textarea').blur(function(){
		$('#us_panel').css({'position':'fixed','bottom':'0'});
		$('.post_comment_content').removeClass('show');
	});
	*/
	...
	$('.remove_msg').click(function(){
		...
		$('.delete_check_sure').click(function(){
			if(rmsLock){
				rmsLock = false;
				$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,id:rmsId},function(){
					$('#delete_check').removeClass('show');
	...
	$('.reply_msg_post').click(function(){
		if($(this).hasClass('checked')){
			$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,content:$('#reply_msg_area').val(),nickname:reply_nickname},function(){
				$.get("#",{id:reply_post_id},function(data){
	...
	var choose_cate = ['cate_all','cate_creat','cate_site','cate_inspire','cate_article','cate_app'],choose_sort = ['sort_re','sort_recom','sort_fa','sort_vi','sort_com'];
	

	$('.choose_cate li').each(function(i){
		$(this).find('.s').addClass(choose_cate[i]);
	});
		...
		$(window).bind('load',function(){
			...
			$('#container').bind('scroll',function(){
				...
				if(t>=($('#list').height()-$(window).height())&&scLock==true){
					...
					now_page++;
					scLock = false;
					$.get('#',{page:now_page,cat:dt[0],soindustrys:[dt[1]],sort:dt[2],search:search_value},function(data){
						var v = $(data).find('#list ul').html();
						$('#list ul').append(v);
	...
	$('.more_comment').click(function(){
		$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,lastid:$('#comment li').eq($('#comment li').length-1).attr('postdata'),postid:commentId2},function(data){
		...
		$(window).bind('load',function(){
			...
			$('#container').bind('scroll',function(){
				...
				if(t>=($('#designer_list').height()-$(window).height())&&scLock==true){
					...
					designers_page++;
					scLock = false;
					$.get('#',{page:designers_page,sort:designers_sort},function(data){
	...
	$('.add_fans').click(function(){
		var _this = $(this);
		$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,uid:_this.attr('data-uid')},function(){
	...
	$('.send_msg_to').click(function(){
		if($(this).hasClass('checked')){
			$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,content:$('#send_msg_content').val(),nickname:send_me},function(){
	...
	var mskAnm = setInterval(function(){
		$('.loading_mask ul').removeClass('anm');
		setTimeout(function(){
			$('.loading_mask ul').addClass('anm');
		},1)
	},1500);
	...
	$('.login_submit').click(function(){
		$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,email:$('.login_user input').val(),password:$('.login_password input').val()},function(){
	...
	function replyThis(){
		$('.reply_this').click(function(){
			if(document.getElementById('us_panel2')){
				isReply = true;
				reply_lock = true;
		...
		$('.add_newpost_post').click(function(){
			if(reply_lock==true&&isReply==true&&$('.newpost_w_t textarea').val()!=''){
				reply_lock = false;
				$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,postid:commentId2,postcid:_cid,postuid:comment_id,content:$('.newpost_w_t textarea').val()},function(data){
					var ddd = '<div class="quote"><div class="pd10">'+_ct+'</div></div>';
			...
			if(reply_lock==true&&isReply==false&&$('.newpost_w_t textarea').val()!=''){
				reply_lock = false;
				$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,postid:commentId2,content:$('.newpost_w_t textarea').val()},function(data){
	...
	var editAlbum = (function(){
		...
		$('.edit_this').click(function(){
			...
			$('.edit_album_sure').click(function(){
				$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,id:pid,title:$('.edit_album_input').val()},function(data){
					_this.parent().parent().parent().find('dt .pd5').html($('.edit_album_input').val());
					$('#edit_album').removeClass('show');
				},'json');
			});
			$('.edit_album_delete').click(function(){
				$.post("#",{timestamp:YYToken.timestamp,token:YYToken.token,id:pid},function(data){
